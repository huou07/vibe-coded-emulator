...
VkExtent2D clamp_extent(const VkSurfaceCapabilitiesKHR& capabilities, CGSize drawable_size) {
...
struct VulkanFrontend::Impl {
    explicit Impl(VulkanFrontend* frontend) : owner(frontend) {}

    VulkanFrontend* owner = nullptr;
    ...
    CGSize drawable_size() const {
        if (!layer) return CGSizeMake(1.0, 1.0);
        CGSize size = layer.drawableSize;
        if (size.width > 0.0 && size.height > 0.0) return size;
        const CGFloat scale = std::max<CGFloat>(layer.contentsScale, 1.0);
        return CGSizeMake(std::max<CGFloat>(layer.bounds.size.width * scale, 1.0),
                          std::max<CGFloat>(layer.bounds.size.height * scale, 1.0));
    }

    bool load_runtime(const char* path, std::string& error) {
        if (!path || !*path) {
            error = "The bundled MoltenVK runtime is unavailable.";
            return false;
        }
        ...
        if (!runtime) {
            error = std::string("Cannot load bundled MoltenVK: ") + (dlerror() ?: "unknown loader error");
            return false;
        }
    ...
    bool create_vulkan_instance(std::string& error) {
        ...
        info.enabledExtensionCount = static_cast<uint32_t>(sizeof(extensions) / sizeof(extensions[0]));
        info.ppEnabledExtensionNames = extensions;
        if (create_instance(&info, nullptr, &instance) != VK_SUCCESS || !instance) {
            error = "MoltenVK could not create a Vulkan 1.1 instance.";
            return false;
        }
        destroy_instance = load_instance_function<PFN_vkDestroyInstance>(get_instance_proc_addr, instance, "vkDestroyInstance");
        create_metal_surface = load_instance_function<PFN_vkCreateMetalSurfaceEXT>(get_instance_proc_addr, instance, "vkCreateMetalSurfaceEXT");
        ...
        if (!destroy_instance || !create_metal_surface || !destroy_surface || !enumerate_physical_devices ||
            ...
            error = "MoltenVK is missing a required Vulkan presentation entry point.";
            return false;
        }
        return true;
    }
    ...
    bool create_surface(std::string& error) {
        if (!layer || !layer.device) {
            error = "This Mac does not expose a Metal device for Vulkan presentation.";
            return false;
        }
        VkMetalSurfaceCreateInfoEXT info{};
        info.sType = VK_STRUCTURE_TYPE_METAL_SURFACE_CREATE_INFO_EXT;
        info.pLayer = layer;
        if (create_metal_surface(instance, &info, nullptr, &surface) != VK_SUCCESS || !surface) {
            error = "MoltenVK could not bind the VibeCodedEmulator native surface.";
            return false;
        }
        return true;
    }
    ...
    bool choose_physical_device(std::string& error) {
        ...
        if (enumerate_physical_devices(instance, &count, nullptr) != VK_SUCCESS || !count) {
            error = "MoltenVK did not expose a Vulkan-capable Metal GPU.";
            return false;
        }
        std::vector<VkPhysicalDevice> devices(count);
        if (enumerate_physical_devices(instance, &count, devices.data()) != VK_SUCCESS) {
            error = "MoltenVK could not enumerate the Metal GPU.";
            return false;
        }
        ...
        error = "No Vulkan graphics queue can present into this Mac window.";
        return false;
    }
    ...
    bool create_core_device(const void* raw_negotiation, std::string& error) {
        ...
        if (!negotiation->create_device(&context, instance, physical_device, surface, get_instance_proc_addr,
            ...
            error = "Azahar could not create a Vulkan device through MoltenVK.";
            return false;
        }
    ...
    bool create_software_device(std::string& error) {
        if (!create_device || !get_device_proc_addr || !enumerate_device_extension_properties) {
            error = "MoltenVK is missing the Vulkan device-creation entry points.";
            return false;
        }
        uint32_t extension_count = 0;
        if (enumerate_device_extension_properties(physical_device, nullptr, &extension_count, nullptr) != VK_SUCCESS) {
            error = "MoltenVK could not inspect the Metal GPU device extensions.";
            return false;
        }
        ...
        if (extension_count &&
            ...
            error = "MoltenVK could not enumerate the Metal GPU device extensions.";
            return false;
        }
        ...
        if (!has_extension(VK_KHR_SWAPCHAIN_EXTENSION_NAME)) {
            error = "This Metal GPU does not expose the Vulkan swapchain extension.";
            return false;
        }
        ...
        if (create_device(physical_device, &info, nullptr, &device) != VK_SUCCESS || !device) {
            error = "MoltenVK could not create a Vulkan device for software-frame presentation.";
            return false;
        }
        const auto get_queue = load_device_function<PFN_vkGetDeviceQueue>(get_device_proc_addr, device, "vkGetDeviceQueue");
        if (!get_queue) {
            error = "MoltenVK is missing vkGetDeviceQueue.";
            return false;
        }
        get_queue(device, queue_family_index, 0, &queue);
        if (!queue) {
            error = "MoltenVK did not provide a graphics queue for software-frame presentation.";
            return false;
        }
    ...
    bool load_device_functions(std::string& error) {
        ...
        if (!destroy_device || !device_wait_idle || !create_swapchain || !destroy_swapchain ||
            ...
            !get_image_memory_requirements || !bind_image_memory || !cmd_copy_buffer_to_image) {
            error = "MoltenVK is missing a required Vulkan device entry point.";
            return false;
        }
        return true;
    }
    ...
    bool create_core_drain_resources(std::string& error) {
        ...
        if (create_command_pool(device, &pool_info, nullptr, &core_drain_command_pool) != VK_SUCCESS ||
            ...
            error = "MoltenVK could not create CoreVulkan recovery commands.";
            return false;
        }
        ...
        if (allocate_command_buffers(device, &command_info, &core_drain_command) != VK_SUCCESS || !core_drain_command) {
            error = "MoltenVK could not allocate CoreVulkan recovery commands.";
        ...
        if (create_fence(device, &fence_info, nullptr, &core_drain_fence) != VK_SUCCESS || !core_drain_fence) {
            error = "MoltenVK could not create CoreVulkan recovery synchronization.";
            destroy_core_drain_resources();
            return false;
        }
        return true;
    ...
    bool create_software_command_resources(std::string& error) {
        ...
        if (create_command_pool(device, &pool_info, nullptr, &software_command_pool) != VK_SUCCESS || !software_command_pool) {
            error = "MoltenVK could not create a software-frame upload command pool.";
            return false;
        }
        ...
        if (allocate_command_buffers(device, &command_info, commands.data()) != VK_SUCCESS) {
            error = "MoltenVK could not allocate software-frame upload command buffers.";
        ...
        for (uint32_t index = 0; index < kFrameRingSize; ++index) {
            ...
            if (create_semaphore(device, &semaphore_info, nullptr, &software_slots[index].upload_finished) != VK_SUCCESS ||
                ...
                error = "MoltenVK could not create software-frame upload synchronization.";
                destroy_software_resources();
                return false;
            }
        }
    ...
    bool create_software_resources(SoftwareFrameSlot& slot, uint32_t width, uint32_t height,
        ...
        if (create_buffer(device, &buffer_info, nullptr, &slot.staging_buffer) != VK_SUCCESS || !slot.staging_buffer) {
            error = "MoltenVK could not create a software-frame staging buffer.";
            destroy_software_slot_resources(slot);
            return false;
        }
        ...
        if (staging_memory_type == std::numeric_limits<uint32_t>::max()) {
            error = "This Metal GPU does not expose coherent host memory for software-frame uploads.";
            destroy_software_slot_resources(slot);
            return false;
        }
        ...
        if (allocate_memory(device, &staging_allocation, nullptr, &slot.staging_memory) != VK_SUCCESS ||
            ...
            map_memory(device, slot.staging_memory, 0, staging_size, 0, &slot.staging_mapping) != VK_SUCCESS ||
            !slot.staging_mapping) {
            error = "MoltenVK could not map a software-frame staging buffer.";
            destroy_software_slot_resources(slot);
            return false;
        }
        ...
        if (create_image(device, &image_info, nullptr, &slot.image) != VK_SUCCESS || !slot.image) {
            error = "MoltenVK could not create a software-frame Vulkan image.";
            destroy_software_slot_resources(slot);
            return false;
        }
        ...
        if (image_memory_type == std::numeric_limits<uint32_t>::max()) {
            error = "This Metal GPU does not expose memory for software-frame Vulkan images.";
            destroy_software_slot_resources(slot);
            return false;
        }
        ...
        if (allocate_memory(device, &image_allocation, nullptr, &slot.image_memory) != VK_SUCCESS ||
            ...
            error = "MoltenVK could not allocate a software-frame Vulkan image.";
            destroy_software_slot_resources(slot);
            return false;
        }
    ...
    bool create_swapchain_resources(std::string& error) {
        ...
        if (get_surface_capabilities(physical_device, surface, &capabilities) != VK_SUCCESS) {
            error = "MoltenVK could not inspect the native window surface.";
            return false;
        }
        if (!(capabilities.supportedUsageFlags & VK_IMAGE_USAGE_TRANSFER_DST_BIT)) {
            error = "This Metal surface cannot accept Vulkan frame transfers.";
            return false;
        }
        uint32_t format_count = 0;
        get_surface_formats(physical_device, surface, &format_count, nullptr);
        if (!format_count) {
            error = "MoltenVK did not provide a swapchain color format.";
            return false;
        }
        std::vector<VkSurfaceFormatKHR> formats(format_count);
        get_surface_formats(physical_device, surface, &format_count, formats.data());
        // Vulkan specifies VK_FORMAT_UNDEFINED as "the application chooses".
        // MoltenVK normally returns BGRA8 UNORM for our CAMetalLayer, but make
        // that fallback explicit instead of querying format properties for the
        // undefined sentinel during an unusual window/display reconfiguration.
        ...
        if (formats.size() == 1 && selected.format == VK_FORMAT_UNDEFINED) {
            ...
            for (const VkFormat preferred : {VK_FORMAT_B8G8R8A8_UNORM, VK_FORMAT_B8G8R8A8_SRGB}) {
                const auto candidate = std::find_if(formats.begin(), formats.end(), [preferred](const VkSurfaceFormatKHR& item) {
                    return item.format == preferred;
                });
                if (candidate != formats.end()) {
                    VkFormatProperties properties{};
                    get_format_properties(physical_device, candidate->format, &properties);
        ...
        if (!(selected_properties.optimalTilingFeatures & VK_FORMAT_FEATURE_BLIT_DST_BIT)) {
            error = "This Metal surface does not provide a Vulkan blit-compatible color format.";
            return false;
        }
        ...
        if (create_command_pool(device, &pool_info, nullptr, &command_pool) != VK_SUCCESS) {
            error = "MoltenVK could not create a presentation command pool.";
            return false;
        }
        ...
        if (allocate_command_buffers(device, &buffers, command_buffers.data()) != VK_SUCCESS) {
            error = "MoltenVK could not allocate presentation command buffers.";
            return false;
        }
        VkSemaphoreCreateInfo semaphore_info{};
        semaphore_info.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;
        ...
        for (auto& frame : presentation_frames) {
            if (create_semaphore(device, &semaphore_info, nullptr, &frame.image_available) != VK_SUCCESS ||
                ...
                error = "MoltenVK could not create presentation synchronization primitives.";
                return false;
            }
        }
        extent = desired;
    ...
    bool recreate_swapchain_if_needed(std::string& error) {
        ...
        const CGSize size = drawable_size();
    ...
    bool initialize(void* metal_layer, const char* moltenvk_path,
        ...
        layer.device = layer.device ?: MTLCreateSystemDefaultDevice();
        layer.pixelFormat = MTLPixelFormatBGRA8Unorm;
        // The presenter uses a Vulkan blit into each CAMetalLayer drawable.
        // `framebufferOnly` would forbid that transfer path on Metal.
        layer.framebufferOnly = NO;
        if (!load_runtime(moltenvk_path, error) || !create_vulkan_instance(error) || !create_surface(error) ||
        ...
        interface = {
            ...
            [](void* handle, const RetroVulkanImage* image, uint32_t semaphore_count,
               const VkSemaphore* semaphores, uint32_t source_queue_family) {
                static_cast<VulkanFrontend*>(handle)->receive_image(image, semaphore_count, semaphores, source_queue_family);
            },
            [](void* handle) { return static_cast<VulkanFrontend*>(handle)->sync_index(); },
            [](void* handle) { return static_cast<VulkanFrontend*>(handle)->sync_index_mask(); },
            nullptr,
            [](void* handle) { static_cast<VulkanFrontend*>(handle)->wait_sync_index(); },
            [](void* handle) { static_cast<VulkanFrontend*>(handle)->lock_queue(); },
            [](void* handle) { static_cast<VulkanFrontend*>(handle)->unlock_queue(); },
            nullptr,
        };
        active = hardware_callbacks.context_type == kRetroHwContextVulkan;
        if (!active) {
            error = "Azahar requested a non-Vulkan renderer on macOS.";
            shutdown();
            return false;
        }
    ...
    bool initialize_software(void* metal_layer, const char* moltenvk_path, std::string& error) {
        ...
        layer.device = layer.device ?: MTLCreateSystemDefaultDevice();
        layer.pixelFormat = MTLPixelFormatBGRA8Unorm;
        // The staging image is copied into this layer's Vulkan swapchain.
        layer.framebufferOnly = NO;
        if (!load_runtime(moltenvk_path, error) || !create_vulkan_instance(error) || !create_surface(error) ||
                ...
                for (unsigned y = 0; y < height; ++y) {
                    ...
                    for (unsigned x = 0; x < width; ++x) {
                        ...
                        if (pixel_format == kRetroPixelFormatXRGB8888) {
                            ...
                            output[2] = expand_5_bit((pixel >> 10) & 0x1fu);
                        }
                        output[3] = 0xffu;
                    }
                }
...
VulkanFrontend::VulkanFrontend() : impl_(std::make_unique<Impl>(this)) {}
VulkanFrontend::~VulkanFrontend() = default;

bool VulkanFrontend::initialize(void* metal_layer, const char* moltenvk_path,
                                const RetroHwRenderCallback& hardware_callbacks,
                                const void* negotiation_interface, std::string& error) {
    return impl_->initialize(metal_layer, moltenvk_path, hardware_callbacks, negotiation_interface, error);
}

bool VulkanFrontend::initialize_software(void* metal_layer, const char* moltenvk_path, std::string& error) {
    return impl_->initialize_software(metal_layer, moltenvk_path, error);
}

void VulkanFrontend::shutdown() { impl_->shutdown(); }
bool VulkanFrontend::ready() const { return impl_->active; }
void* VulkanFrontend::hardware_interface() { return impl_->active ? &impl_->interface : nullptr; }
uint32_t VulkanFrontend::sync_index() const { return impl_->current_sync_index.load(std::memory_order_relaxed); }
uint32_t VulkanFrontend::sync_index_mask() const { return 0x3u; }
uint64_t VulkanFrontend::presented_frames() const { return impl_->frame_count.load(std::memory_order_relaxed); }
NativeRendererMetrics VulkanFrontend::renderer_metrics() const {
    std::scoped_lock lock{impl_->mutex};
    return impl_->renderer_metrics();
}

void VulkanFrontend::receive_image(const void* image, uint32_t semaphore_count,
...
void VulkanFrontend::present(unsigned width, unsigned height) { impl_->present(width, height); }

void VulkanFrontend::present_software(const void* framebuffer, unsigned width, unsigned height,
                                      std::size_t pitch, int pixel_format) {
    impl_->present_software(framebuffer, width, height, pitch, pixel_format);
}

bool VulkanFrontend::acquire_software_framebuffer(unsigned width, unsigned height, int pixel_format,
                                                   void*& data, std::size_t& pitch) {
    return impl_->acquire_software_framebuffer(width, height, pixel_format, data, pitch);
}

bool VulkanFrontend::discard_pending_core_frame() {
    std::scoped_lock lock{impl_->mutex};
    return impl_->discard_pending_core_frame();
}

void VulkanFrontend::wait_sync_index() {
    std::scoped_lock lock{impl_->mutex};
...
void VulkanFrontend::lock_queue() { impl_->mutex.lock(); }
void VulkanFrontend::unlock_queue() { impl_->mutex.unlock(); }
void VulkanFrontend::set_signal_semaphore(const void*) {}
void VulkanFrontend::set_command_buffers(uint32_t, const void*) {}
