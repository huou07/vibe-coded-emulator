...
(() => {
  ...
  const reconcileNativeLibrary = async () => {
    if (!nativeOfflineApp) return;
    const invoke=nativeBridge().invoke;
    ...
    for (const item of Array.isArray(manifest?.items)?manifest.items:[]) {
      ...
      if (!/^[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}$/i.test(id)||!name) continue;
      const previous=existing.get(id),nativeRecord=/Android/i.test(navigator.userAgent)?{nativePath:`/native-rom/${name}`}:{nativeUrl:`/_an3/rom/${name}`};
      if (previous?.romHash===item.contentHash&&Number(previous?.size)===Number(item.size)&&nativeRecord.nativePath===previous?.nativePath&&nativeRecord.nativeUrl===previous?.nativeUrl) continue;
      ...
      [...groups.entries()].sort(([left],[right])=>(systems[left]?.label||left).localeCompare(systems[right]?.label||right)).forEach(([system,items])=>{
        ...
        items.forEach(game=>{
          ...
          const repairNativeRom=async()=>{
            ...
            const repaired={...game,id,title:cleanTitle(imported.name||game.title),name:imported.name||game.name,system,size:imported.size||game.size,romHash:imported.romHash||game.romHash||"",addedAt:Date.now(),nativeRomId:id,...(imported.path?{nativePath:imported.path}:{})};
            await put(repaired);await renderLibrary();
            const launch=nativeIntegratedLauncher();
            if(launch)await launch(id,system,"preserve",repaired.size);
            return true;
          };
          card.className="game-card offline-game-card";card.dataset.system=game.system;card.dataset.romId=nativeRomId||"";card.dataset.testid=game.system==="switch"?"switch-game-card":"game-card";body.className="game-card-body";meta.className="game-meta";badge.className="badge";badge.textContent=systems[game.system]?.label||game.system;meta.appendChild(badge);title.textContent=game.title;size.className="muted";size.textContent=formatSize(game.size ?? game.file?.size ?? 0);actions.className="card-actions";
    ...
    const configureNativePicker=()=>{
      ...
      nativePicker.addEventListener("click",async()=>{
        ...
        try{
          ...
          const title=cleanTitle(imported.name),game={id,title,name:imported.name,system,size:imported.size,romHash:imported.romHash||"",addedAt:Date.now(),nativeRomId:id,...(imported.path?{nativePath:imported.path}:{nativeUrl:imported.url})};
