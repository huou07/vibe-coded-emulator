...
[[ -f "$TMP/app.py" && -d "$TMP/static" && -f "$TMP/deploy/an3-arcade.service" && -f "$TMP/deploy/backup-production-current.sh" && -f "$TMP/deploy/rollback-production.sh" && -f "$TMP/deploy/seed-azahar-core.sh" && -f "$TMP/deploy/rollback-azahar-core.sh" && -f "$TMP/deploy/retire-production-rom-library.sh" && -f "$TMP/tools/verify-release-catalog.py" && -f "$TMP/netcode.py" && -f "$TMP/sync_engine.py" && -f "$TMP/qrcodegen.py" && -f "$TMP/LICENSE" && -f "$TMP/THIRD_PARTY_NOTICES.md" && -f "$TMP/native-offline/releases/catalog.json" ]] || {
