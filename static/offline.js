...
(() => {
  ...
  const nativeSystemShortName = system => ({gba:"GBA",nds:"NDS","3ds":"3DS"}[system]||String(system||"").toUpperCase());
  ...
  const renderLibrary = async () => {
    ...
    try {
      const games=await all();grid.replaceChildren();updateReadiness(games.length);
