...
class AndroidNativeRuntimeTests(unittest.TestCase):
    ...
    def test_android_menu_retains_dmg_controls_tabs_and_explicit_save_settings(self):
        ...
        self.assertIn('"Save and Exit"', overlay)
        ...
        self.assertIn("fun footerButton(", overlay)
    ...
    def test_android_native_autosave_uses_the_single_shared_mode(self):
        ...
        self.assertIn("fun resolveValues(values: Map<String, Any?>): String", resolver)
    ...
    def test_recreated_activity_resumes_instead_of_cold_booting(self):
        # A recreated activity is a new instance whose `surfaceStarted` is false
        # even though the runtime still holds a valid suspend snapshot. The
        # instance flag must survive onSaveInstanceState so surfaceCreated asks
        # the runtime to resume instead of cold-booting the ROM.
        activity = source(GAME_ACTIVITY)
        self.assertIn('STATE_RESUMED = "an3-native-surface-resumed"', activity)
        self.assertIn("savedInstanceState?.getBoolean(STATE_RESUMED) == true", activity)
        self.assertIn("surfaceStarted || resumeRequested", activity)
        self.assertIn("outState.putBoolean(STATE_RESUMED, surfaceStarted)", activity)
        surface = between(activity, "override fun surfaceCreated", "override fun surfaceChanged")
        self.assertIn("resumeRequested = false", surface)

    def test_process_death_recovers_from_the_on_disk_suspend_snapshot(self):
        # A suspend snapshot written by a previous process must be importable
        # after process death even though the in-process `resumable_save_path`
        # is gone. A failed export removes the file, so a present file is a
        # successful snapshot.
        jni = source(JNI)
        self.assertIn(
            'resumable_save_path==saves || std::filesystem::exists(saves+"/.suspend.state")',
            jni,
        )
    ...
    def test_native_upload_paths_are_bounded_and_have_no_gpu_readback_or_idle_hot_path(self):
        ...
        # The GL backend must not read back the GPU in the per-frame path. The
        # only permitted exception is the explicitly opt-in, rate-limited
        # physical-device saturation diagnostic (debug.an3.gl_saturation): it
        # must be gated so an ordinary frame never triggers a readback.
        self.assertNotIn("glFinish", gles)
        present_gl = between(gles, "bool AndroidGles3Backend::present_gl_frame", "NativeVideoStatus AndroidGles3Backend::metrics")
        if "glReadPixels" in present_gl:
            self.assertIn("gl_saturation_diagnostic_enabled()", present_gl)
            self.assertIn("present_count % 60u", present_gl)
    ...
    def test_android_3ds_pins_the_faithful_software_pica_shader_path(self):
        host = source(HOST)
        # RG-125: the Azahar OpenGL ES hardware-shader path renders 3D content
        # black on Adreno GPUs, so the runtime must keep the software PICA
        # shader interpreter for Android 3DS sessions.
        self.assertIn(
            '!std::strcmp(variable->key, "citra_use_hw_shader")) variable->value="disabled";', host
        )

    def test_android_3ds_vulkan_avoids_the_stale_driver_pipeline_cache(self):
        host = source(HOST)
        # RG-081: on the Vulkan renderer a persisted core driver pipeline cache
        # makes Adreno's shader compiler fail, so vkCreateGraphicsPipelines
        # returns VK_ERROR_UNKNOWN and the uncaught vk::UnknownError aborts the
        # process. The runtime must keep the Vulkan 3DS path off that cache.
    ...
    def test_android_layout_and_virtual_controls_share_one_model_and_one_next_action(self):
        ...
        # Movement options (including the circular D-pad) come from the canonical
        # action model instead of hard-coded duplicates.
        self.assertIn("NativeInputActions.directionalControls", overlay)
        self.assertIn("NativeInputActions.directionalControlLabels", overlay)
